...
#######################################################################################
# Parameters
year1 = 2011 # Change year
filepath = f"/home/hbito/cascadia_obs_ensemble_backup/data/picks_{year1}_122-129/"  # Change file path 
...
time1 = datetime.datetime(year=year1,month=1,day=1) # Change the start date
time2 = datetime.datetime(year=year1+1,month=1,day=1) # Change the end date 
time_bins = pd.to_datetime(np.arange(time1,time2,pd.Timedelta(1,'days')))

inventory = client_inventory.get_stations(network="C8,7D,7A,CN,NV,UW,UO,NC,BK,TA,OO,PB,X6,Z5,X9", station="*", minlatitude=40,minlongitude=-129,maxlatitude=50,maxlongitude=-122, starttime=time1.strftime('%Y%m%d'),endtime=time2.strftime('%Y%m%d')) # Change the networks, lat and lon.
#######################################################################################
