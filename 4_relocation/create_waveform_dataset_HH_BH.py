...
for (event_id, network, station), group in islice(group_iter):
    print("-" * 50)
...
with open('/home/hbito/cascadia_obs_ensemble_backup/data/datasets_all_regions/waveforms_HH_BH.pkl', 'wb') as f:
...
with h5py.File(output_waveform_file_HH_BH, "w") as f:
    for bucket, traces in waveform_buckets_HH_BH.items():
        ...
        try:
            ...
            print(f"Failed to write bucket {bucket}: {e}")
            save_errors.append({
                "bucket": bucket,
    ...
    if save_errors:
        err_path = output_waveform_file_HH_BH.replace(".h5", "_save_errors.csv")
        pd.DataFrame(save_errors).to_csv(err_path, index=False)
        print(f"Some data failed to save. See error log at: {err_path}")
