...
output_waveform_file_HH_BH = "/wd1/hbito_data/data/datasets_all_regions/waveforms_HH_BH_all.h5"
output_metadata_file_HH_BH = "/wd1/hbito_data/data/datasets_all_regions/metadata_HH_BH_all.csv"
...
for (event_id, network, station), group in tqdm(islice(group_iter,None),total=len(group_iter)):
#     print("-" * 50)
...
with h5py.File(output_waveform_file_HH_BH, "w") as f:
    for bucket, traces in waveform_buckets_HH_BH.items():
        ...
        try:
            ...
            print(f"Failed to save bucket {bucket} to HDF5: {e}")
            h5_errors.append({
                'bucket': bucket,
...
# Save HDF5 writing errors if any
if h5_errors:
    error_path = output_waveform_file_HH_BH.replace(".h5", "_write_errors.csv")
    pd.DataFrame(h5_errors).to_csv(error_path, index=False)
    print(f"Saved HDF5 writing errors to {error_path}")
else:
    print("All waveform data saved successfully to HDF5.")
