...
output_waveform_file_HH_BH = "/wd1/hbito_data/data/datasets_all_regions/waveforms_HH_BH_mod.h5"
output_metadata_file_HH_BH = "/wd1/hbito_data/data/datasets_all_regions/metadata_HH_BH_mod.csv"
output_waveform_file_EH = "/wd1/hbito_data/data/datasets_all_regions/waveforms_EH_mod.h5"
output_metadata_file_EH = "/wd1/hbito_data/data/datasets_all_regions/metadata_EH_mod.csv"
...
for (event_id, network, station), group in tqdm(df.groupby(['event_id', 'network', 'station']), total=len(df.groupby(['event_id', 'network', 'station'])), desc="Processing events"):
    ...
    try:
        if network in ['NC', 'BK']:
            _waveform = get_waveform_across_midnight(client_ncedc, network, station, "*", "*", trace_start1, trace_end1, event_id=event_id)
        else:
            _waveform = get_waveform_across_midnight(client_waveform, network, station, "*", "?H?", trace_start1, trace_end1, event_id=event_id)
...
with open('/wd1/hbito_data/data/datasets_all_regions/waveform_buckets_HH_BH_mod.pkl', 'wb') as f:
...
with h5py.File(output_waveform_file_EH, "w") as f:
    for bucket, traces in waveform_buckets_EH.items():
        if traces:
            f.create_dataset(f"/data/{bucket}", data=np.stack(traces, axis=0), dtype="float32")
with open('/wd1/hbito_data/data/datasets_all_regions/waveform_buckets_EH_mod.pkl', 'wb') as f:
