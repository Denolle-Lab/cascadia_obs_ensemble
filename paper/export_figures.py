...
# (paper/figures/<dest>, script, <src>.png the script writes, dir it writes into)
SCRIPT_FIGURES = [
    ("fig7_catalog_ml_confidence.png", "4_relocation/magnitude/phase5_pygmt_map.py",
     "cascadia_ML_map_confidence.png", ROOT / "data" / "magnitude"),
]
...
def main() -> int:
    ...
    if args.execute:
        ...
        for _dest, script, _src, _sd in SCRIPT_FIGURES:
            print(f"executing {script} ...")
            # scripts use paths relative to their own dir -> run from there
            r = subprocess.run(["python", (ROOT / script).name], cwd=str((ROOT / script).parent))
            if r.returncode:
                print(f"  WARN {script} failed (exit {r.returncode})")

    ok = miss = 0
    collect = [(dest, src_dir / src) for dest, _nb, src in FIGURES]
    collect += [(dest, sd / src) for dest, _sc, src, sd in SCRIPT_FIGURES]
