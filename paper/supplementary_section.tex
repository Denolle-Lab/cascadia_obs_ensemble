...
\section{Magnitude completeness by region}

Figure~\ref{fig:supp_fmd} compares the regional frequency-magnitude distributions of the
ensemble catalog (local magnitude $M_L$) with ANSS/ComCat. Offshore, the ocean-bottom
network lowers the magnitude of completeness by roughly one to two units relative to
the land-based ANSS (both compared as local magnitudes, ANSS restricted to ml-typed
events; over all relocated events): $M_c\approx1.3$--$1.6$ at the Mendocino and Gorda
segments, $1.7$ at the Blanco transform, and $1.8$ at the Endeavour segment, versus
$M_c\approx3.2$ for the few ml-typed ANSS events offshore, with Gutenberg--Richter
$b\sim0.9$--$1.4$ (max-curvature $M_c$, Aki--Utsu $b$).
Onshore (Puget Sound) the two catalogs are comparable ($M_c\approx1.4$). Our $M_L$
saturates for the largest events, so ANSS retains more of the high-magnitude tail; the
gain here is at the small-magnitude, offshore end, which is where the network is
designed to contribute.

\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{figures/supp_completeness.png}
    \caption{{\bf Frequency-magnitude distributions by region}, ensemble $M_L$ (red) vs
    ANSS (blue), with max-curvature $M_c$ (dotted) and Aki--Utsu $b$. The offshore
    segments show the largest completeness gains.}
    \label{fig:supp_fmd}
\end{figure}
