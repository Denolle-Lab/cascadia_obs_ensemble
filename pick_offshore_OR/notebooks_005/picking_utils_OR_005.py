...
#     channels = 'HH?,HN?,BH?' 
    ...
    try:
        ...
        if network in ['NC', 'BK']:
            print('test18')
            # Query waveforms
            _sdata = client_ncedc.get_waveforms(network=network, station=station, location="*", channel=channels,starttime=UTCDateTime(t1), endtime=UTCDateTime(t2))
            
            print('test20')
            
        else:
            print('test19')
            print(network,station,channels,t1)
            _sdata = client_waveform.get_waveforms(network=network, station=station, channel=channels, starttime=UTCDateTime(t1), endtime=UTCDateTime(t2))
