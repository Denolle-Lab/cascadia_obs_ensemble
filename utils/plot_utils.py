...
def subplot_waveforms(picks,dir,fig_title):
    ...
    for event_idx in tqdm(picks.idx.unique(), total=len(picks.idx.unique())):
        ...
        for i,item in enumerate(distances):
            ...
            has_EH = bool(st_sta.select(channel="EH?"))
            ...
            for ax in range(len(_st)):
                ...
                # Add the label only once per channel
                # print(i)
                if len(_st)==3 and count_3 == 0:
                    count_3 += 1
                    station_code_3 = [network_code,station_code]
                elif len(_st)==2 and count_2 == 0:
                    count_2 += 1
                    station_code_2 = [network_code,station_code]
                elif len(_st)==1 and count_1 == 0:
                    count_1 += 1
                    station_code_1 = [network_code,station_code]
                else:
                    pass
            ...
            m.plot(x_sta, y_sta, '^', color='blue', markersize=6.5)
        
        if count_3 == 1:
            st_sta_legend = st.select(network=station_code_3[0],station=station_code_3[1])
            ...
            for ax, tr in enumerate(_st):
                axs[ax].plot([], [], label=f"{tr.stats.channel[2]}")
                axs[ax].legend(loc='upper right',handlelength=0, handletextpad=0) 
        elif count_2 == 1:
            st_sta_legend = st.select(network=station_code_2[0],station=station_code_2[1])
            # Define the desired order of channels
            desired_order = {
                'Z': ['HHZ', 'BHZ','EHZ'],
                'N': ['HHN', 'HH1', 'BHN', 'BH1', 'EHN', 'EN1'],
            ...
            st_sta_legend = st.select(network=station_code_1[0],station=station_code_1[1])
            # Define the desired order of channels
            desired_order = {
                'Z': ['HHZ', 'BHZ','EHZ'],
                'N': ['HHN', 'HH1', 'BHN', 'BH1', 'EHN', 'EN1'],
        ...
        m.plot(x_event, y_event, 'yo', markersize=6.5)

        # fig.supxlabel('Time [sec]', y=0.02)
        fig.supylabel('Epicentral Distance [km]',x=0.09)
        fig.suptitle(f"{fig_title}: \nOrigin Time={otime}, Latitude={round(olat, 2)}, Longtitude={round(olon, 2)}, Depth={round(odepth, 2)}", y=0.96)

        
        fig.savefig(path,format='png')  

def subplot_waveforms_no_title(picks,dir):
